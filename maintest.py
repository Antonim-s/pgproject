...
class Snake:
    def __init__(self, mesto, h, w):
        ...
        if RECORD >= 30:
            ...
            chislo = '1'
        self.ru = load_image(f'snake_parts/ru{chislo}.png', colorkey=-1)
        self.rd = load_image(f'snake_parts/rd{chislo}.png', colorkey=-1)
        self.lu = load_image(f'snake_parts/lu{chislo}.png', colorkey=-1)
        self.ld = load_image(f'snake_parts/ld{chislo}.png', colorkey=-1)
        self.gor = load_image(f'snake_parts/goriz{chislo}.png', colorkey=-1)
        self.vert = load_image(f'snake_parts/vert{chislo}.png', colorkey=-1)
        self.hu = load_image(f'snake_parts/hu{chislo}.png', colorkey=-1)
        self.hd = load_image(f'snake_parts/hd{chislo}.png', colorkey=-1)
        self.hr = load_image(f'snake_parts/hr{chislo}.png', colorkey=-1)
        self.hl = load_image(f'snake_parts/hl{chislo}.png', colorkey=-1)
        self.tu = load_image(f'snake_parts/tu{chislo}.png', colorkey=-1)
        self.td = load_image(f'snake_parts/td{chislo}.png', colorkey=-1)
        self.tr = load_image(f'snake_parts/tr{chislo}.png', colorkey=-1)
        self.tl = load_image(f'snake_parts/tl{chislo}.png', colorkey=-1)
            ...
            if (((x - 1 == x_1 or x - 1 + self.board_size[1] == x_1) and (
                ...
                return self.gor
                    ...
                    y + 1 == y1 or y + 1 - self.board_size[0] == y1)) or (
                          ...
                          (y_1 == y + 1 or y + 1 - self.board_size[0] == y_1))) and x == x1 == x_1:
                return self.vert
    ...
    def move(self, board, kuda=None):
        if self.dead:
            return
        if kuda is None and self.last_move is not None:
            kuda = self.last_move
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if key == pygame.K_ESCAPE:
                    ...
                    string2 = font.render(f'RECORD: {RECORD}', 1, pygame.Color('white'))
                    sur.blit(string, (10, 10))
                    sur.blit(string2, (10, 40))
                    screen.blit(sur, (0, 0))
                    pygame.display.flip()
                    while True:
                ...
                if board.update(screen, key):
                    ...
                    text = 'GAME OVER'
                    text1 = f'SCORE: {score}'
                    text2 = f'RECORD: {RECORD}'
                    with open('data/rec.txt', 'w') as rec:
                        rec.write(str(RECORD))

                    string1 = font.render(text1, 1, pygame.Color('white'))
                    string = font.render(text, 1, pygame.Color('white'))
                    string2 = font.render(text2, 1, pygame.Color('white'))
                    sur.blit(string, (10, 10))
                    sur.blit(string1, (10, 40))
                    sur.blit(string2, (10, 80))
                    screen.blit(sur, (0, 0))
                    pygame.display.flip()
                    while True:
                        for ev in pygame.event.get():
