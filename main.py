...
class Apple:
    ...
    def generate_new_pos(self, board):
        y, x = self.board_size
        while True:
            xx = randrange(0, x, 1)
            yy = randrange(0, y, 1)
...
class Snake:
    ...
    def lengthen(self):
        if self.dead:
            return
        prexvostx, prexvosty = self.mesto[1]
        xvostx, xvosty = self.mesto[0]
        if xvosty < prexvosty:
            self.mesto = [(xvostx, xvosty + 1)] + self.mesto
        elif xvosty > prexvosty:
            self.mesto = [(xvostx, xvosty - 1)] + self.mesto
        elif xvostx < prexvostx:
            self.mesto = [(xvostx - 1, xvosty)] + self.mesto
        elif xvostx > prexvostx:
            self.mesto = [(xvostx + 1, xvosty)] + self.mesto
...
class Board:
    ...
    def __init__(self, map):
        ...
        self.apple.generate_new_pos(self.board)
        self.left = 0
        self.top = 0
        self.cell_size = 30
    ...
    def render(self, screen):
        for y in range(len(self.board)):
            for x in range(len(self.board[0])):
                if self.board[y][x] == 1:
                    pygame.draw.rect(screen, CLR['snake'],
                                     (x * self.cell_size + self.left,
                                      y * self.cell_size + self.top,
                                      self.cell_size, self.cell_size))
                elif self.board[y][x] == 2:
                    pygame.draw.rect(screen, CLR['apple'],
                                     (x * self.cell_size + self.left,
                                      y * self.cell_size + self.top,
                                      self.cell_size, self.cell_size))
                elif self.board[y][x] == 3:
                    pygame.draw.rect(screen, CLR['title'],
                                     (x * self.cell_size + self.left,
                                      y * self.cell_size + self.top,
                                      self.cell_size, self.cell_size))
    ...
    def update(self, screen, key):
        ...
        for x, y in self.snake.mesto:
            if (x, y) == self.apple.pos:
                self.snake.lengthen()
                self.apple.generate_new_pos(boardd)
                SOUNDNOM.play()
            boardd[y][x] = 1
